...
func (s *Session) RefreshDiffs() {
	...
	for _, snap := range snapshots {
		...
		if snap.status == "added" || snap.status == "untracked" {
			hunks = vcs.FileDiffUnifiedNewFile(snap.content)
		} else if vc != nil {
			h, err := vc.FileDiffUnified(snap.path, baseRef, repoRoot, false)
			if err != nil {
...
func (s *Session) RefreshFileList() {
	...
	// Shell out to vcs.VCS for changed files — no lock held.
	s.mu.RLock()
	baseRef := s.BaseRef
	repoRoot := s.RepoRoot
	s.mu.RUnlock()
	...
	changes = config.FilterIgnored(changes, s.IgnorePatterns)
	...
	if len(changes) > lazyFileThreshold {
		if baseRef != "" {
			numstats, _ = vc.DiffNumstat(baseRef, repoRoot)
		}
	}
...
func (s *Session) watchGit(stop <-chan struct{}) {
	...
	// Read vcs.VCS once under lock — it doesn't change after session init.
	s.mu.RLock()
	vc := s.VCS
	s.mu.RUnlock()
...
func (s *Session) buildFileEntry(absPath, repoRoot, baseRef string, vc vcs.VCS) *FileEntry {
	...
	if vc != nil {
		hunks, diffErr := vc.FileDiffUnified(relPath, baseRef, repoRoot, false)
		if diffErr == nil {
			fe.DiffHunks = hunks
		}
	}
...
func (s *Session) refreshFilesFromCLIDirs() {
	...
	for _, absPath := range expandedPaths {
		...
		if fe := s.buildFileEntry(absPath, repoRoot, baseRef, vc); fe != nil {
...
func (s *Session) carryForwardFileComments(f *FileEntry) {
	...
	newLines := diff.SplitLines(currContent)
